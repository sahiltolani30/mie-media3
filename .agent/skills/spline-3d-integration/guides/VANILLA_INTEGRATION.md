...
Install the dependency locally and use a bundler:
...
### Import Maps (Vanilla HTML without bundler)

If you aren't using a bundler (Vite, Webpack), use an importmap to map the bare specifier to your locally installed package:

```html
<script type="importmap">
  {
    "imports": {
      "@splinetool/runtime": "/path/to/local/runtime.js"
    }
  }
</script>
<script type="module">
...
<html lang="en">
  <head>
    ...
    <!-- Optional: Use an importmap to resolve bare specifiers locally without a bundler -->
    <script type="importmap">
      {
        "imports": {
          "@splinetool/runtime": "/node_modules/@splinetool/runtime/build/runtime.js"
        }
      }
    </script>
  </head>
  <body>
    ...
    <script type="module">
      ...
      spline
        .load("https://prod.spline.design/YOUR_SCENE_ID/scene.splinecode")
        .then(() => {
...
spline.addEventListener("keyDown", (e) => {
  // SECURITY WARNING: e.target.name comes from the Spline scene.
  // Treat as untrusted input and sanitize before DOM insertion/execution.
...
// SECURITY WARNING: 'score' comes from the Spline scene and could be manipulated.
// Treat as untrusted input and sanitize before passing to innerHTML or eval.
...
<canvas
  id="canvas3d"
  style="width:100vw; height:100vh; opacity:0; transition: opacity 0.5s;"
></canvas>

<script type="module">
  // Requires a bundler or importmap for bare specifiers
  import { Application } from "@splinetool/runtime";
  ...
  spline
    .load("https://prod.spline.design/YOUR_ID/scene.splinecode")
    .then(() => {
