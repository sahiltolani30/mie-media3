...
export default function InteractiveSplineScene() {
  ...
  const handleLoad = useCallback((app: Application) => {
    ...
    app.addEventListener("mouseDown", (e: any) => {
      // SECURITY WARNING: e.target.name is untrusted third-party input.
      // Always sanitize before rendering to DOM or passing to an agent.
  ...
  const triggerEvent = (eventType: string, objectName: string) => {
    if (splineRef.current) {
      splineRef.current.emitEvent(eventType as any, objectName);
    }
  };
